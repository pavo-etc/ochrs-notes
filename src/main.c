...
#define LOCATION "Perth, Australia"
...
fppict(FILE *f, char *filename, char *caption, int header, int link)
{
	...
	if(!isVideo) {
		...
		for(i = 0; i < 3; i++) {
			...
			scat(dst, buf);
			scat(dst, ".png");
			img = getfile(fakepath, name, dst, "r");
			if(!img) {
				printf("(fppict 2) Couldn't open image %s%s%s\n", path, name, dst);
				continue;
			}
			if(count > 0)
				scat(srcset, ", ");
...
fpnavsub(FILE *f, Term *t, Term *target)
{
	...
	for(i = 0; i < t->children_len; ++i) {
		...
		if(tc->name == target->name)
			fprintf(f, "<li><a href='%s.html' class=\"active\">%s</a></li>\n", tc->filename, tc->name);
		else
...
fphtml(FILE *f, Glossary *glo, Lexicon *lex, Term *t, Journal *jou)
{
	...
	fputs("<hr />\n", f);
	fputs("<section>\n", f);
	fputs("<p>Made ineffeciently by <a href='https://zachmanson.com'>Zach Manson</a></p>\n", f);
	fputs("</section>\n", f);
	fputs("</footer>\n", f);
	fputs("</body>\n</html>\n", f);
	fclose(f);
...
fprss(FILE *f, Journal *jou)
{
	...
	for(i = 0; i < jou->len; ++i) {
		...
		fputs("</pubDate>\n", f);
		fputs("  <dc:creator><![CDATA[Zach Manson]]></dc:creator>\n", f);
		fputs("  <description>\n", f);
		fputs("<![CDATA[", f);
		if(l.pict)
...
parse_journal(FILE *fp, Block *block, Lexicon *lex, Journal *jou)
{
	...
	while(fgets(line, 1024, fp)) {
		...
		if(img) {
			...
			return error("(parse_journal) Couldn't open image", path);
		if(len >= 29)
